...
try{
 ...
 if(!oldSync.test(code))throw Error('[Runner] syncAccounting marker not found');
 code=code.replace(oldSync,newSync);
 const oldRisk=/function riskBlocked\(\)\{[\s\S]*?\n\}\nfunction eventUsed/;
 const newRisk=`function riskBlocked(){\n  resetDaily();\n  const now=Date.now();\n  if(Number(state.cooldownUntil||0)>0){\n    if(now>=Number(state.cooldownUntil)){state.cooldownUntil=0;state.halted=false;state.consecutiveLosses=0;saveState();console.log('[RISK] 30-minute cooldown complete; trading resumed');return false;}\n    state.halted=true;saveState();return true;\n  }\n  if(state.halted){state.halted=false;saveState();}\n  return false;\n}\nfunction eventUsed`;
 if(!oldRisk.test(code))throw Error('[Runner] riskBlocked marker not found');
 ...
 const newExit='state.position=null;syncAccounting();if(pnl<0){state.consecutiveLosses=Number(state.consecutiveLosses||0)+1;}else if(pnl>0){state.consecutiveLosses=0;}if(state.consecutiveLosses>=MAX_CONSECUTIVE_LOSSES){state.halted=true;state.cooldownUntil=Date.now()+COOLDOWN_MS;console.log(`[RISK] ${state.consecutiveLosses} consecutive losses; cooldown 30 minutes`);}saveState();';
 if(!code.includes(oldExit))throw Error('[Runner] exit risk marker not found');
 code=code.replace(oldExit,newExit);
 const startup=`\n/* Restart-safe risk state: historical losses never trigger a new cooldown. */\nif(Number(state.cooldownUntil||0)<=Date.now()){state.cooldownUntil=0;state.halted=false;state.consecutiveLosses=0;saveState();console.log('[RISK] startup reset: historical consecutive losses cleared');}\nelse{state.halted=true;console.log('[RISK] startup: existing cooldown preserved');}\n`;
 const listen='app.listen(';if(!code.includes(listen))throw Error('[Runner] app.listen marker not found');
 const webhook=`\n/* Telegram webhook for commands; outbound trade notifications use Bot API directly. */\nconst __tgToken=String(process.env.TELEGRAM_BOT_TOKEN||'').trim().replace(/[\\\"']/g,'');\nconst __tgBase=String(process.env.TELEGRAM_WEBHOOK_URL||process.env.RENDER_EXTERNAL_URL||'').trim().replace(/\\/$/,'');\nasync function __tgSend(chatId,text){if(!__tgToken||!chatId)return;try{await fetch('https://api.telegram.org/bot'+__tgToken+'/sendMessage',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:chatId,text})});}catch(e){console.error('[Telegram send]',e.message||e);}}\nif(typeof app!=='undefined'){app.post('/telegram/webhook',async(req,res)=>{try{const msg=req.body&&req.body.message;const chatId=msg&&msg.chat&&msg.chat.id;const text=String(msg&&msg.text||'').trim();if(chatId&&/^\\/(start|help)(?:@[^\\s]+)?$/i.test(text))await __tgSend(chatId,'OKX Event Bot\\n\\n模式：PAPER（模擬盤）\\n\\n/stat 指令可查詢統計。');res.sendStatus(200);}catch(e){console.error('[Telegram webhook]',e.message||e);res.sendStatus(200);}});}\nif(__tgToken&&__tgBase){const __u=__tgBase+'/telegram/webhook';setTimeout(()=>fetch('https://api.telegram.org/bot'+__tgToken+'/deleteWebhook',{method:'POST'}).then(()=>fetch('https://api.telegram.org/bot'+__tgToken+'/setWebhook',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url:__u,drop_pending_updates:true})})).then(()=>console.log('[Telegram] Webhook enabled:',__u)).catch(e=>console.error('[Telegram webhook setup]',e.message||e)),500);}\n`;
 ...
 console.log('[Runner] Risk: 3 consecutive NEW losses -> 30 minute cooldown -> reset counter -> auto resume');
 console.log('[Runner] Historical trades do not trigger cooldown on restart.');
 console.log('[Runner] Telegram polling disabled; webhook enabled for commands.');
