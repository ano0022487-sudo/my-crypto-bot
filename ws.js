...
class OKXPublicWS{
 constructor(onEvent=()=>{}){
  this.ws=null;this.timer=null;this.pingTimer=null;this.instIds=[];this.reconnectAttempt=0;this.reconnectScheduled=false;this.connected=false;this.stopping=false;this.subscriptions=new Set();this.subscribedArgs=new Set();this.subscribedInstruments=new Set();this.unsupported=new Set();this.subscribing=false;this.socketGeneration=0;this.connectionCount=0;this.reconnectCount=0;this.lastMessageAt=null;this.lastTradeAt=null;this.lastTickerAt=null;this.lastSuccessfulMessageAt=null;this.timeoutCount=0;this.lastErrorLogAt=0;this.lastReconnectLogAt=0;
 ...
 connect(){
  ...
  this.ws=ws;this.connectionCount+=1;this.subscriptions.clear();this.subscribedArgs.clear();this.subscribedInstruments.clear();this.subscribing=false;
  ...
  ws.on('close',(code,reason)=>{if(generation!==this.socketGeneration)return;this.connected=false;this.stopHeartbeat();this.subscriptions.clear();this.subscribedArgs.clear();this.subscribedInstruments.clear();this.subscribing=false;this.ws=null;this.scheduleReconnect(`close:${code}${reason?.toString()?`:${reason.toString().slice(0,80)}`:''}`);this.rateLimitedReconnect('OKX public WebSocket disconnected',{code,reason:reason?.toString()||'',backoff:this.nextReconnectDelay()});});
 }
 ...
 startHeartbeat(ws,generation){
  this.stopHeartbeat();this.pingTimer=setInterval(()=>{
   if(generation!==this.socketGeneration||ws!==this.ws||ws.readyState!==WebSocket.OPEN)return;
   const age=Date.now()-(this.lastMessageAt||Date.now());if(age<config.WS_HEARTBEAT_MS)return;
   ...
   ws._okxPongDeadline=Date.now()+config.WS_HEARTBEAT_MS;try{ws.send('ping');}catch(error){this.rateLimitedError('OKX public WebSocket ping failed',{error:error.message});this.cleanupSocket(ws);this.ws=null;this.connected=false;this.stopHeartbeat();this.scheduleReconnect('ping-failed');}
  },5000);
 }
 ...
 async subscribe(ws,generation){
  ...
  args.push({channel:'liquidation-orders',instType:'SWAP'});const size=Math.max(1,config.WS_SUBSCRIBE_BATCH_SIZE);
 ...
 onMessage(raw){
  ...
  if(msg.event==='subscribe'){if(msg.arg){this.subscribedArgs.add(JSON.stringify(msg.arg));if(msg.arg.channel)this.subscribedChannelsSet=this.subscribedChannelsSet||new Set();this.subscribedChannelsSet.add(msg.arg.channel);if(msg.arg.instId)this.subscribedInstruments.add(msg.arg.instId);}return;}
 ...
 snapshot(){const channels=this.subscribedChannelsSet?this.subscribedChannelsSet.size:0;return {connected:this.connected,connectionCount:this.connectionCount,activeSockets:this.ws?1:0,subscriptionRequests:this.subscriptions.size,subscribedChannels:channels,subscribedArguments:this.subscribedArgs.size,subscribedInstruments:this.subscribedInstruments.size||0,lastTradeTimestamp:this.lastTradeAt,lastTickerTimestamp:this.lastTickerAt,lastSuccessfulMessageTimestamp:this.lastSuccessfulMessageAt,reconnectCount:this.reconnectCount,timeoutCount:this.timeoutCount,staleSubscriptionCount:this.staleSubscriptions(),unsupportedSubscriptions:this.unsupported.size,openInterest:Object.fromEntries(this.latest.openInterest),funding:Object.fromEntries(this.latest.funding),liquidations:this.latest.liquidations,tickers:Object.fromEntries(this.latest.tickers)};}
}
