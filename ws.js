...
class OKXPublicWS{
 constructor(onEvent=()=>{}){
  this.ws=null;this.timer=null;this.pingTimer=null;this.instIds=[];this.reconnectAttempt=0;this.reconnectScheduled=false;this.connected=false;this.stopping=false;this.subscriptions=new Set();this.subscribedArgs=new Set();this.unsupported=new Set();this.subscribing=false;this.socketGeneration=0;this.connectionCount=0;this.reconnectCount=0;this.lastMessageAt=null;this.lastTradeAt=null;this.lastTickerAt=null;this.lastSuccessfulMessageAt=null;this.timeoutCount=0;this.lastErrorLogAt=0;this.lastReconnectLogAt=0;
 ...
 stop(){this.stopping=true;this.clearReconnectTimer();this.stopHeartbeat();const ws=this.ws;this.ws=null;this.connected=false;this.cleanupSocket(ws);this.subscriptions.clear();this.subscribedArgs.clear();}
 ...
 connect(){
  ...
  const ws=new WebSocket(config.OKX_PUBLIC_WS_URL,{handshakeTimeout:config.REQUEST_TIMEOUT_MS});
  this.ws=ws;this.connectionCount+=1;this.subscriptions.clear();this.subscribedArgs.clear();this.subscribing=false;
  ...
  ws.on('close',(code,reason)=>{if(generation!==this.socketGeneration)return;this.connected=false;this.stopHeartbeat();this.subscriptions.clear();this.subscribedArgs.clear();this.subscribing=false;this.ws=null;this.scheduleReconnect(`close:${code}${reason?.toString()?`:${reason.toString().slice(0,80)}`:''}`);this.rateLimitedReconnect('OKX public WebSocket disconnected',{code,reason:reason?.toString()||'',backoff:this.nextReconnectDelay()});});
 }
 ...
 scheduleReconnect(reason){
  if(this.stopping||this.reconnectScheduled)return;
  this.reconnectScheduled=true;const delay=this.nextReconnectDelay();this.reconnectAttempt+=1;this.reconnectCount+=1;this.lastReconnectLogAt=Date.now();
  this.timer=setTimeout(()=>{this.timer=null;this.reconnectScheduled=false;if(!this.stopping)this.connect();},delay);
  logger.warn('OKX WebSocket reconnect scheduled',{reason,attempt:this.reconnectAttempt,delay});
 }
 startHeartbeat(ws,generation){
  this.stopHeartbeat();
  this.pingTimer=setInterval(()=>{
   if(generation!==this.socketGeneration||ws!==this.ws||ws.readyState!==WebSocket.OPEN)return;
   const age=Date.now()-(this.lastMessageAt||Date.now());
   if(age<config.WS_HEARTBEAT_MS)return;
   if(ws._okxPongDeadline&&Date.now()>ws._okxPongDeadline){this.timeoutCount+=1;this.rateLimitedError('OKX public WebSocket heartbeat timeout',{timeoutCount:this.timeoutCount});this.cleanupSocket(ws);this.ws=null;this.connected=false;this.stopHeartbeat();this.scheduleReconnect('heartbeat-timeout');return;}
   ws._okxPongDeadline=Date.now()+config.WS_HEARTBEAT_MS;
 ...
 async subscribe(ws,generation){
  if(this.subscribing||!ws||ws.readyState!==WebSocket.OPEN)return;
  this.subscribing=true;const args=[];
  for(const id of this.instIds){for(const channel of ['tickers','trades','candle5m','open-interest','funding-rate']){if(!this.unsupported.has(`${channel}:${id}`))args.push({channel,instId:id});}}
  args.push({channel:'liquidation-orders',instType:'SWAP'});
  const size=Math.max(1,config.WS_SUBSCRIBE_BATCH_SIZE);
  try{
   for(let i=0;i<args.length;i+=size){
    if(generation!==this.socketGeneration||ws!==this.ws||ws.readyState!==WebSocket.OPEN)break;
    const batch=args.slice(i,i+size);const key=JSON.stringify(batch);
    if(this.subscriptions.has(key))continue;
    ws.send(JSON.stringify({op:'subscribe',args:batch}));this.subscriptions.add(key);
    if(i+size<args.length)await new Promise(resolve=>setTimeout(resolve,config.WS_SUBSCRIBE_BATCH_DELAY_MS));
   }
 ...
 onMessage(raw){
  ...
  if(text==='ping'){if(this.ws?.readyState===WebSocket.OPEN)this.ws.send('pong');return;}
  if(text==='pong'){if(this.ws)this.ws._okxPongDeadline=null;return;}
  let msg;try{msg=JSON.parse(text);}catch{return;}
  if(msg.event==='subscribe'){if(msg.arg)this.subscribedArgs.add(JSON.stringify(msg.arg));return;}
  if(msg.event==='error'){const detail=String(msg.msg||'');const channel=msg.arg?.channel;const instId=msg.arg?.instId;const match=detail.match(/channel:([^,\s]+),instId:([^\s]+) doesn't exist/i);if(channel&&instId)this.unsupported.add(`${channel}:${instId}`);else if(match)this.unsupported.add(`${match[1]}:${match[2]}`);this.rateLimitedError('OKX public WebSocket subscription rejected',{code:msg.code,message:detail,channel:channel||match?.[1]||null,instId:instId||match?.[2]||null});return;}
 ...
 staleSubscriptions(){const now=Date.now();let count=0;for(const id of this.instIds){const ticker=this.lastTickerByInstrument.get(id)||0;if(!ticker||now-ticker>config.COLLECTOR_STALE_MS)count+=1;}return count;}
 snapshot(){return {connected:this.connected,connectionCount:this.connectionCount,activeSockets:this.ws?1:0,subscriptionRequests:this.subscriptions.size,subscribedChannels:this.subscribedArgs.size,subscribedInstruments:this.instIds.length,lastTradeTimestamp:this.lastTradeAt,lastTickerTimestamp:this.lastTickerAt,lastSuccessfulMessageTimestamp:this.lastSuccessfulMessageAt,reconnectCount:this.reconnectCount,timeoutCount:this.timeoutCount,staleSubscriptionCount:this.staleSubscriptions(),unsupportedSubscriptions:this.unsupported.size,openInterest:Object.fromEntries(this.latest.openInterest),funding:Object.fromEntries(this.latest.funding),liquidations:this.latest.liquidations,tickers:Object.fromEntries(this.latest.tickers)};}
}
