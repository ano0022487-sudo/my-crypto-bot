...
fs.readFileSync=function(file,encoding,...rest){
  ...
  /* Probability is deliberately capped below the old artificial 92.4% output.
     It is a confidence score, not a claim of calibrated historical win rate. */
  const probability=Math.min(0.84,Math.max(0.60,0.60+Math.min(0.24,gap*0.006)));
  const score=Math.round(Math.min(100,60+gap*0.8+(best>=80?8:0)));
...
`;
    ...
    /* Safety calibration: never allow the old synthetic 90%+ probability mapping
       to survive if another source injects it later in the loaded bot. */
    code=code.replace(/probUp=Math\\.min\\(\\.94,Math\\.max\\(\\.06,.5\\+\\(up-down\\)\\*\\.008\\)\\)/g,"probUp=Math.min(.84,Math.max(.60,.60+Math.min(.24,Math.max(0,best-opp)*.006)))");
    code=code.replace(/probUp=Math\\.min\\(\\.94,Math\\.max\\(\\.06,.5\\+\\(up-down\\)\\*\\.0065\\)\\)/g,"probUp=Math.min(.84,Math.max(.60,.60+Math.min(.24,Math.max(0,best-opposing)*.006)))");
