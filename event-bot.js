...
let __mainLoopBusy=false;
let __started=false;
async function mainLoop(){if(__mainLoopBusy){console.error('[MAIN LOOP] Duplicate invocation blocked');return;}__mainLoopBusy=true;try{console.log(`[HEARTBEAT] 1H mathematical trend ${new Date().toISOString()}`);if(riskBlocked()||state.position)return;const candidates=await scanCandidates();if(!candidates.length){console.log('[SCAN RESULT] candidates=0');return;}await placeCandidate(candidates[0]);}catch(e){console.error('[MAIN LOOP]',e.stack||e);}finally{__mainLoopBusy=false;}}
async function start(){if(__started){console.error('[START] Duplicate start blocked');return;}__started=true;console.log(`OKX EVENT CONTRACT BOT RUNNING ON PORT ${PORT}`);console.log(`[SAFETY] ${LIVE_TRADING?'LIVE':'PAPER'} / 1U / 1H math trend / trend-following only / Score>=${MIN_SCORE} / Model>=${MIN_MODEL_PROB*100}% / Edge>=${MIN_EDGE*100}% / Entry ${MIN_ENTRY_PRICE}-${MAX_ENTRY_PRICE}`);app.get('/',(req,res)=>res.status(200).send('OKX EVENT CONTRACT BOT RUNNING'));app.get('/health',(req,res)=>res.json({ok:true,mode:LIVE_TRADING?'LIVE':'PAPER',strategy:'1H mathematical trend only',position:Boolean(state.position),cooldownUntil:state.cooldownUntil||0}));app.listen(PORT,()=>console.log(`HTTP listening on ${PORT}`));if(bot){bot.onText(/^\/(stats|stat|統計)$/i,async msg=>{if(String(msg.chat.id)!==TG_CHAT)return;syncAccounting();const ts=state.trades||[],wins=ts.filter(t=>Number(t.pnl)>0).length,losses=ts.filter(t=>Number(t.pnl)<0).length,pnl=ts.reduce((s,t)=>s+Number(t.pnl||0),0),wr=ts.length?wins/ts.length*100:0;await notify(`📊 PAPER 統計\n\n交易筆數：${ts.length}\n勝場：${wins}\n敗場：${losses}\n勝率：${wr.toFixed(1)}%\n累計 PnL：${pnl>=0?'+':''}${pnl.toFixed(4)}U\n目前資金：${Number(state.paperEquity||START_CAPITAL).toFixed(4)}U\n目前連敗：${state.consecutiveLosses}\n停機鎖定：${state.halted?'是':'否'}`);});}await mainLoop();setInterval(()=>{void mainLoop();},CHECK_INTERVAL);setInterval(()=>{void managePosition();},POSITION_CHECK_INTERVAL);}
