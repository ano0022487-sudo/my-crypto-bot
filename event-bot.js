...
function modelProbability(price,strike,c5,c15){const cl5=closes(c5),cl15=closes(c15),vol=volumes(c5),p5=pivots(c5),p15=pivots(c15);const e20_5=ema(cl5,20),e50_5=ema(cl5,50),e20_15=ema(cl15,20),e50_15=ema(cl15,50),r=rsi(cl5),a=atr(c5),avg=vol.slice(-20).reduce((x,y)=>x+y,0)/(Math.min(20,vol.length)||1),vr=avg>0?(vol.at(-1)||0)/avg:1;let up=0,down=0;const reasons=[];if(e20_5&&e50_5){if(e20_5>e50_5){up+=10;reasons.push('5m up');}else{down+=10;reasons.push('5m down');}}if(e20_15&&e50_15){if(e20_15>e50_15){up+=15;reasons.push('15m up');}else{down+=15;reasons.push('15m down');}}if(r!==null){if(r>=55&&r<=72){up+=10;reasons.push('RSI bullish');}else if(r<=45&&r>=28){down+=10;reasons.push('RSI bearish');}}if(vr>=1.15){if(up>=down)up+=6;else down+=6;reasons.push(`volume ${vr.toFixed(2)}x`);}const resistance=nearestAbove([...p5.resistance,...p15.resistance],price);if(resistance&&price>=resistance*0.999){up+=5;reasons.push('resistance test');}if(strike&&a){const dist=(price-strike)/a;if(dist>=0.75){up+=10;reasons.push('strike distance');}else if(dist<=-0.75){down+=10;reasons.push('strike distance');}}const total=up+down;let probUp=0.5;if(total>0)probUp=Math.min(0.94,Math.max(0.06,0.5+(up-down)*0.0075));const score=Math.round(Math.min(100,50+Math.max(up,down)*1.55));return{score,upProbability:probUp,downProbability:1-probUp,reasons,atr:a,volumeRatio:vr,direction:up>=down?'UP':'DOWN'};}
...
async function scanCandidates(){const instruments=await discoverEventInstruments();const filtered=instruments.filter(inst=>{const coin=getBaseAsset(inst);return coin&&isUpDown(inst)&&(!inst.state||String(inst.state).toLowerCase()==='live')&&allowedExpiry(inst)&&!eventUsed(inst.instId);});console.log(`[SCAN DISCOVERY] instruments=${instruments.length}`);console.log(`[SCAN FILTER] live-updown=${filtered.length}`);const candidates=[],cache={};for(const inst of filtered){try{const coin=getBaseAsset(inst),underlying=UNDERLYING_MAP[coin];if(!cache[coin]){const [c5,c15]=await Promise.all([getCandles(underlying,'5m',100),getCandles(underlying,'15m',100)]);cache[coin]={c5,c15};}const {c5,c15}=cache[coin];if(!c5.length||!c15.length)continue;const t=await getTicker(inst.instId,'EVENTS');const yesAsk=Number(t?.askPx||t?.last),yesBid=Number(t?.bidPx||t?.last);if(!(yesAsk>0&&yesBid>0&&yesAsk<1&&yesBid>0))continue;const price=Number(c5.at(-1)?.[4]);if(!Number.isFinite(price))continue;const strike=getStrike(inst),model=modelProbability(price,strike,c5,c15),yesEntry=yesAsk,noEntry=1-yesBid;if(!validPrice(yesEntry)||!validPrice(noEntry))continue;const yesEdge=model.upProbability-yesEntry,noEdge=model.downProbability-noEntry;const side=yesEdge>=noEdge?'yes':'no';const entryPx=side==='yes'?yesEntry:noEntry;const modelProb=side==='yes'?model.upProbability:model.downProbability;const edge=side==='yes'?yesEdge:noEdge;const mins=minutesToExpiry(inst);let reject=null;if(model.score<MIN_SCORE)reject=`score ${model.score}<${MIN_SCORE}`;else if(modelProb<MIN_MODEL_PROB)reject=`model ${(modelProb*100).toFixed(1)}%<${MIN_MODEL_PROB*100}%`;else if(edge<MIN_EDGE)reject=`edge ${(edge*100).toFixed(1)}%<${MIN_EDGE*100}%`;if(reject){console.log('[EVENT REJECT]',JSON.stringify({instId:inst.instId,reason:reject,side,score:model.score,model:Number((modelProb*100).toFixed(1)),market:Number((entryPx*100).toFixed(1)),edge:Number((edge*100).toFixed(1)),mins:Number(mins?.toFixed(1))}));continue;}candidates.push({inst,seriesId:inst.seriesId,coin,underlying,side,entryPx,modelProb,marketProb:entryPx,edge,score:model.score,reasons:model.reasons,strikePx:strike,underlyingPrice:price,expiry:getExpiry(inst),minutesToExpiry:mins,atr:model.atr});console.log('[EVENT PASS]',JSON.stringify({instId:inst.instId,side,entryPx,score:model.score,model:Number((modelProb*100).toFixed(1)),edge:Number((edge*100).toFixed(1)),reasons:model.reasons}));}catch(e){console.error(`[EVENT CANDIDATE ERROR] ${inst.instId}:`,e.message);}}console.log(`[SCAN DIAGNOSTICS] filtered=${filtered.length} rejected=${filtered.length-candidates.length} passed=${candidates.length}`);return candidates.sort((a,b)=>b.edge-a.edge||b.score-a.score);}
