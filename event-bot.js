...
/*
===========================================================
 OKX EVENT CONTRACT SNR ROLLING BOT (5U FIXED + 3X COMPOUND)
===========================================================
*/
...
async function publicGet(pathname, params = {}) {
  ...
  const response = await request({ method: 'GET', url: `${BASE_URL}${requestPath}` });
  if (!response.data || String(response.data.code) !== '0') {
    throw new Error(`OKX public error ${response.status}: ` + JSON.stringify(response.data));
  }
...
function sign(timestamp, method, requestPath, body) {
  return crypto.createHmac('sha256', SECRET_KEY).update(timestamp + method.toUpperCase() + requestPath + body).digest('base64');
}

async function privateRequest(method, requestPath, bodyObj = null) {
  if (!API_KEY || !SECRET_KEY || !PASSPHRASE) {
    throw new Error('OKX API credentials missing');
  }
  ...
  if (!response.data || String(response.data.code) !== '0') {
    throw new Error(`OKX private error ${response.status}: ` + JSON.stringify(response.data));
  }
  return Array.isArray(response.data.data) ? response.data.data : [];
}

/* =========================================================
   STATE (ADDED COMPOUNDING TRACKERS)
========================================================= */
function freshState() {
  return {
...
function resetDaily() {
  ...
  if (state.day !== today) {
    state.day = today;
    state.startEquity = LIVE_TRADING ? 0 : Number(state.paperEquity || START_CAPITAL);
    state.realizedPnl = 0;
...
function confirmed(candles) {
  return candles.slice().sort((a, b) => Number(a[0]) - Number(b[0])).filter(x => String(x[8]) === '1');
}
...
function pivots(candles, lookback = 3) {
  ...
  for (let i = lookback; i < candles.length - lookback; i++) {
    const highest = Math.max(...h.slice(i - lookback, i + lookback + 1));
    const lowest = Math.min(...l.slice(i - lookback, i + lookback + 1));
    if (h[i] === highest) resistance.push(h[i]);
    if (l[i] === lowest) support.push(l[i]);
  }
  return { resistance: resistance.slice(-10), support: support.slice(-10) };
}
...
async function getTicker(instId, instType = null) {
  const params = { instId };
  if (instType) params.instType = instType;
  const rows = await publicGet('/api/v5/market/ticker', params);
  return rows?.[0] || null;
}

/* =========================================================
   EVENT DISCOVERY (UNCHANGED)
========================================================= */
function getConfiguredSeries() {
  if (!EVENT_SERIES) return [];
  return EVENT_SERIES.split(',').map(x => x.trim()).filter(Boolean);
}

function generatedSeries() {
  const result = [];
  for (const coin of ASSETS) {
    result.push(`${coin}-UPDOWN-15MIN`);
  }
  return result;
}

function parseExpiryFromInstId(instId) {
  const text = String(instId || '').toUpperCase();
  const match = text.match(/-(\d{6})-(\d{4})-(\d{4})$/);
  ...
  const hh = Number(endPart.slice(0, 2));
  const min = Number(endPart.slice(2, 4));
  const year = 2000 + yy;
  return Date.UTC(year, mm - 1, dd, hh, min, 0, 0);
}

function getExpiry(inst) {
  const values = [Number(inst.expTime), Number(inst.expiryTime), Number(inst.endTime)].filter(Number.isFinite);
  if (values.length) return Math.max(...values);
  return parseExpiryFromInstId(inst.instId);
}

function minutesToExpiry(inst) {
  const expiry = getExpiry(inst);
...
async function getEventInstruments(seriesId) {
  return publicGet('/api/v5/public/instruments', { instType: 'EVENTS', seriesId });
}

async function discoverEventInstruments() {
  let series = getConfiguredSeries();
  if (!series.length && AUTO_DISCOVER_SERIES) {
    series = generatedSeries();
  }
  if (!series.length) throw new Error('No EVENT series configured');
  const all = [];
  for (const seriesId of series) {
    try {
      const rows = await getEventInstruments(seriesId);
      if (Array.isArray(rows)) {
        for (const inst of rows) all.push({ ...inst, seriesId: inst.seriesId || seriesId });
      }
    } catch (err) {
      console.error(`[EVENT] ${seriesId}:`, err.message);
    }
  }
  return all;
}
...
function getBaseAsset(inst) {
  const text = `${inst.baseCcy || ''} ${inst.instId || ''} ${inst.seriesId || ''}`.toUpperCase();
  for (const coin of ASSETS) {
...
function getStrike(inst) {
  const fields = ['stk', 'strike', 'strikePx', 'targetPx', 'triggerPx'];
  for (const key of fields) {
    const value = Number(inst[key]);
...
function validPrice(price) {
  return (Number.isFinite(price) && price >= MIN_ENTRY_PRICE && price <= MAX_ENTRY_PRICE);
}

/* =========================================================
   MODEL (UNCHANGED)
========================================================= */
function modelProbability(price, strikePx, c5, c15) {
  const p5 = pivots(c5);
  ...
  const recentVolume = vol5.slice(-20);
  const avgVolume = recentVolume.length ? recentVolume.reduce((a, b) => a + b, 0) / recentVolume.length : 0;
  const currentVolume = vol5[vol5.length - 1] || 0;
  const volumeRatio = avgVolume > 0 ? currentVolume / avgVolume : 1;
  let score = 50;
  const reasons = [];

  if (ema20_5 && ema50_5 && ema20_5 > ema50_5) { score += 10; reasons.push('5m trend'); }
  if (ema20_15 && ema50_15 && ema20_15 > ema50_15) { score += 15; reasons.push('15m trend'); }
  if (rsi5 !== null && rsi5 >= 55 && rsi5 <= 72) { score += 10; reasons.push('RSI'); }
  if (volumeRatio >= 1.15) { score += 8; reasons.push('volume'); }

  const resistance = nearestAbove([...p5.resistance, ...p15.resistance], price);
  if (resistance && price >= resistance * 0.999) { score += 8; reasons.push('SNR resistance test'); }
  if (strikePx && strikePx > 0) {
    if (price > strikePx) { score += 8; reasons.push('above strike'); }
  }
  score = Math.max(0, Math.min(100, score));
  const probability = Math.max(0.50, Math.min(0.80, 0.50 + (score - 50) * 0.006));
  const downProbability = 1 - probability;

  return { score, upProbability: probability, downProbability, reasons };
}
...
async function getEquity() {
  if (!LIVE_TRADING) return Math.max(0, Number(state.paperEquity || START_CAPITAL));
...
async function scanCandidates() {
  ...
  const filtered = instruments.filter(inst => {
    ...
    if (!coin || !isUpDown(inst)) return false;
    if (inst.state && String(inst.state).toLowerCase() !== 'live') return false;
    return allowedExpiry(inst);
  });
  const candidates = [];
  const marketCache = {};
  ...
  for (const inst of filtered) {
    try {
      ...
      if (!marketCache[coin]) {
        const [c5, c15] = await Promise.all([ getCandles(underlying, '5m', 100), getCandles(underlying, '15m', 100) ]);
        marketCache[coin] = { c5, c15 };
      }
      const { c5, c15 } = marketCache[coin];
      if (!c5.length || !c15.length) continue;

      const ticker = await getTicker(inst.instId, 'EVENTS');
      const yesAsk = Number(ticker?.askPx || ticker?.last);
      const yesBid = Number(ticker?.bidPx || ticker?.last);
      if (!(yesAsk > 0 && yesBid > 0)) continue;
      if (yesAsk >= 1 || yesBid <= 0) continue;

      const underlyingPrice = Number(c5[c5.length - 1][4]);
      if (!Number.isFinite(underlyingPrice)) continue;

      const strikePx = getStrike(inst);
      const model = modelProbability(underlyingPrice, strikePx, c5, c15);
      const yesModel = model.upProbability;
      const noModel = model.downProbability;
      const yesEntry = yesAsk;
      const noEntry = 1 - yesBid;

      if (!validPrice(yesEntry) || !validPrice(noEntry)) continue;

      const yesEdge = yesModel - yesEntry;
      const noEdge = noModel - noEntry;

      let side, entryPx, modelProb, marketProb, edge;
      if (yesEdge >= noEdge) {
        side = 'yes'; entryPx = yesEntry; modelProb = yesModel; marketProb = yesEntry; edge = yesEdge;
      } else {
        side = 'no'; entryPx = noEntry; modelProb = noModel; marketProb = noEntry; edge = noEdge;
      }
      if (model.score < MIN_SCORE || edge < MIN_EDGE) continue;

      candidates.push({
        inst, seriesId: inst.seriesId, coin, underlying, side, entryPx, modelProb, marketProb, edge, score: model.score, reasons: model.reasons, strikePx, underlyingPrice, expiry: getExpiry(inst), minutesToExpiry: minutesToExpiry(inst)
      });
    } catch (err) {
      console.error(`[EVENT CANDIDATE ERROR] ${inst.instId}:`, err.message);
    }
  }
...
async function placeEventOrder(candidate, stakeTarget) {
  ...
  if (!(candidate.entryPx > 0 && candidate.entryPx < 1)) {
    throw new Error(`Invalid event price: ${candidate.entryPx}`);
  }
...
async function getOrder(instId, ordId) {
  const requestPath = `/api/v5/trade/order?` + q({ instId, ordId });
  const rows = await privateRequest('GET', requestPath);
  return (rows?.[0] || null);
}
...
async function closePosition(position, currentPx) {
  ...
  const tickSz = Number(inst.tickSz || 0.001);
  const px = roundToTick(currentPx, tickSz);
  const sz = Number(position.sz);

  if (!(sz > 0)) throw new Error(`Invalid close size: ${sz}`);
  ...
  if (!LIVE_TRADING) {
    const pnl = (px - position.entryPx) * sz;
...
async function managePosition() {
  ...
  try {
    const ticker = await getTicker(position.inst.instId, 'EVENTS');
    const yesBid = Number(ticker?.bidPx || ticker?.last);
    const yesAsk = Number(ticker?.askPx || ticker?.last);
    if (!(yesBid > 0)) return;

    let currentPx;
    if (position.side === 'yes') currentPx = yesBid;
    else currentPx = 1 - (yesAsk > 0 ? yesAsk : yesBid);

    if (!(currentPx > 0 && currentPx < 1)) return;

    const change = (currentPx - position.entryPx) / position.entryPx;
    if (change >= EARLY_TP_PCT) {
      await exitPosition(position, currentPx, 'TP');
      return;
    }
    if (change <= -EARLY_SL_PCT) {
      await exitPosition(position, currentPx, 'SL');
    }
  } catch (err) {
    console.error('[EVENT POSITION MANAGER]', err.message);
  }
}
...
async function exitPosition(position, currentPx, reason) {
  ...
  if (pnl > 0) {
    ...
    if (state.compoundingCount >= 3) {
      state.compoundingCount = 0;
      state.nextStake = 5;
    } else {
      state.nextStake = exitPx * position.sz;
    }
  } else {
  ...
  state.trades.push({
    ...
    exitPx, sz: position.sz, pnl, reason
  });
...
async function maybeTrade() {
  ...
  let equity;
  try { equity = await getEquity(); } catch (err) { return; }
  ...
  let candidates;
  try { candidates = await scanCandidates(); } catch (err) { return; }
  if (!candidates.length) return;

  const candidate = candidates[0];

  try {
    const targetStake = state.nextStake > 0 ? state.nextStake : 5;
    const order = await placeEventOrder(candidate, targetStake);

    let filled = order;
    if (LIVE_TRADING && order?.ordId) {
      await sleep(500);
      filled = await getOrder(candidate.inst.instId, order.ordId);
    }
    ...
    state.position = {
      inst: candidate.inst, seriesId: candidate.seriesId, coin: candidate.coin,
      side: candidate.side, sz: fillSz, entryPx: avgPx, stake: actualStake,
      score: candidate.score, edge: candidate.edge, modelProb: candidate.modelProb,
      marketProb: candidate.marketProb, underlyingPrice: candidate.underlyingPrice,
      openedAt: Date.now()
    };
    ...
    console.error('🔴 EVENT ORDER ERROR', err.message);
  }
}
...
/* =========================================================
   EXPRESS ROUTES & LOOP
========================================================= */
app.get('/', (req, res) => { res.json({ ok: true, bot: 'OKX Event Contract SNR Rolling Bot', live: LIVE_TRADING, position: state.position }); });
app.get('/health', async (req, res) => { res.json({ ok: true, live: LIVE_TRADING, time: new Date().toISOString() }); });
...
async function mainLoop() {
  ...
  try {
    ...
    console.error('[EVENT MAIN LOOP]', err.message || err);
  } finally {
    loopRunning = false;
  }
}

app.listen(PORT, async () => { console.log(`OKX EVENT CONTRACT BOT RUNNING ON PORT ${PORT}`); });
