...
class Collector{
 constructor(ws){
  ...
  this.tradeQueue=[];this.tickerQueue=[];this.candleQueue=[];this.oiQueue=[];this.oiLatest=new Map();this.fundingQueue=[];this.fundingLatest=new Map();this.flushInProgress=false;this.analyticsInProgress=false;this.cleanupInProgress=false;this.flushBlockedUntil=0;this.flushFailures=0;this.lastFlushAt=null;this.lastDbErrorAt=null;this.droppedTrades=0;this.droppedTickers=0;this.droppedCandles=0;this.droppedOI=0;this.droppedFunding=0;this.eventsReceived=0;this.lastDbErrorLogAt=0;
 ...
 activeIds(){return this.ids.filter(x=>!this.isUnsupported(x.instId));}
 async start(){
  ...
  this.cleanupTimer=setInterval(()=>this.runCleanup().catch(e=>logger.error('Cleanup failed',{error:e.message})),6*60*60*1000);
 ...
 async handle(type,data){
  ...
  try{
   if(type==='ticker'){
   ...
   }else if(type==='candle5m'){
    const x=normalizeCandle(data);x.symbol=data.instId;
    if(x.timestamp!=null)this.enqueue(this.candleQueue,x,Math.max(1000,this.activeIds().length*4),'droppedCandles');
    ...
    const x=normalizeFunding(data,data.instId);
    if(x?.symbol&&x.timestamp!=null){
      this.fundingLatest.set(x.symbol,x);
      if(this.fundingLatest.size>config.COLLECTOR_MAX_FUNDING_QUEUE){const oldest=this.fundingLatest.keys().next().value;if(oldest){this.fundingLatest.delete(oldest);this.droppedFunding+=1;}}
    }
 ...
 async flushQueues(){
  ...
  for(const [symbol,x] of this.fundingLatest){this.fundingQueue.push(x);this.fundingLatest.delete(symbol);}
  ...
  if(this.fundingQueue.length>config.COLLECTOR_MAX_FUNDING_QUEUE)this.fundingQueue.splice(0,this.fundingQueue.length-config.COLLECTOR_MAX_FUNDING_QUEUE);
  if(!this.tradeQueue.length&&!this.tickerQueue.length&&!this.candleQueue.length&&!this.oiQueue.length&&!this.fundingQueue.length)return;
  this.flushInProgress=true;let trades=[];let tickers=[];let candles=[];let openInterest=[];let funding=[];
  try{
   trades=this.tradeQueue.splice(0,config.COLLECTOR_TRADE_BATCH_SIZE);tickers=this.tickerQueue.splice(0,config.COLLECTOR_TICKER_BATCH_SIZE);candles=this.candleQueue.splice(0,Math.max(100,config.WS_SUBSCRIBE_BATCH_SIZE*2));openInterest=this.oiQueue.splice(0,config.COLLECTOR_OI_BATCH_SIZE);funding=this.fundingQueue.splice(0,config.COLLECTOR_FUNDING_BATCH_SIZE);
   ...
   if(openInterest.length)await db.insertOIBatch(openInterest);
   if(funding.length)await db.insertFundingBatch(funding);
   this.flushFailures=0;this.flushBlockedUntil=0;this.lastFlushAt=Date.now();
  }catch(e){
   if(funding.length)this.fundingQueue.splice(0,0,...funding);
   if(openInterest.length)this.oiQueue.splice(0,0,...openInterest);
   if(candles.length)this.candleQueue.splice(0,0,...candles);
 ...
 async runCleanup(){if(!db.isReady()||this.cleanupInProgress)return;this.cleanupInProgress=true;try{await db.cleanup();}finally{this.cleanupInProgress=false;}}
 async runAnalytics(){
  if(!db.isReady()||this.analyticsInProgress||this.cleanupInProgress)return;
  this.analyticsInProgress=true;
  try{
   const rs=await db.query(`SELECT i.inst_id,t.price,t.ts,t.volume FROM instruments i LEFT JOIN LATERAL(SELECT price,volume,ts FROM ticker_snapshots WHERE inst_id=i.inst_id ORDER BY ts DESC LIMIT 1)t ON true`);
   for(const x of rs.rows){
    if(x.volume==null)continue;
    const base=await db.query(`SELECT volume FROM ticker_snapshots WHERE inst_id=$1 AND ts>now()-interval '24 hours' AND volume IS NOT NULL ORDER BY ts DESC LIMIT $2`,[x.inst_id,config.ANOMALY_BASELINE_POINTS]);
    const z=zscore(x.volume,base.rows.map(r=>r.volume));const severity=anomalySeverity(z,config.ANOMALY_THRESHOLDS);
    if(severity==='正常'||severity==='資料不足')continue;
    const id=await db.insertAnomaly({symbol:x.inst_id,timestamp:new Date(x.ts).getTime(),anomalyType:'volume',severity,measuredValue:x.volume,baseline:{mean:base.rows.length?base.rows.reduce((s,r)=>s+Number(r.volume),0)/base.rows.length:null,zscore:z,sampleSize:base.rows.length},metadata:{source:'ticker_snapshots',definition:'24h historical ticker volume baseline'}});
    if(id)await db.query(`INSERT INTO research_events(anomaly_id,inst_id,t0) VALUES($1,$2,to_timestamp($3/1000.0))`,[id,x.inst_id,new Date(x.ts).getTime()]);
   }
   await db.query(`UPDATE research_events r SET t5_price=(SELECT close FROM candles c WHERE c.inst_id=r.inst_id AND c.bar='5m' AND c.ts>=r.t0+interval '5 minutes' ORDER BY c.ts LIMIT 1),t15_price=(SELECT close FROM candles c WHERE c.inst_id=r.inst_id AND c.bar='5m' AND c.ts>=r.t0+interval '15 minutes' ORDER BY c.ts LIMIT 1),t1h_price=(SELECT close FROM candles c WHERE c.inst_id=r.inst_id AND c.bar='5m' AND c.ts>=r.t0+interval '1 hour' ORDER BY c.ts LIMIT 1),t4h_price=(SELECT close FROM candles c WHERE c.inst_id=r.inst_id AND c.bar='5m' AND c.ts>=r.t0+interval '4 hours' ORDER BY c.ts LIMIT 1),t24h_price=(SELECT close FROM candles c WHERE c.inst_id=r.inst_id AND c.bar='5m' AND c.ts>=r.t0+interval '24 hours' ORDER BY c.ts LIMIT 1),completed_at=CASE WHEN now()>=r.t0+interval '24 hours' THEN now() ELSE completed_at END WHERE r.completed_at IS NULL`);
   await db.query(`UPDATE research_events r SET t5_return=CASE WHEN t5_price IS NOT NULL THEN (t5_price-(SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1))/NULLIF((SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1),0)*100 END,t15_return=CASE WHEN t15_price IS NOT NULL THEN (t15_price-(SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1))/NULLIF((SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1),0)*100 END,t1h_return=CASE WHEN t1h_price IS NOT NULL THEN (t1h_price-(SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1))/NULLIF((SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1),0)*100 END,t4h_return=CASE WHEN t4h_price IS NOT NULL THEN (t4h_price-(SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1))/NULLIF((SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1),0)*100 END,t24h_return=CASE WHEN t24h_price IS NOT NULL THEN (t24h_price-(SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1))/NULLIF((SELECT price FROM ticker_snapshots t WHERE t.inst_id=r.inst_id AND t.ts<=r.t0 ORDER BY t.ts DESC LIMIT 1),0)*100 END WHERE r.completed_at IS NULL OR r.completed_at>now()-interval '24 hours'`);
  }finally{this.analyticsInProgress=false;}
 }
}
