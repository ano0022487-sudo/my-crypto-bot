...
class Collector{
 ...
 async start(){
  ...
  this.flushTimer=setInterval(()=>this.flushQueues().catch(e=>this.recordFlushFailure(e)),config.COLLECTOR_FLUSH_MS);
 ...
 async flushQueues(){
  ...
  try{
   const trades=this.tradeQueue.splice(0,config.COLLECTOR_TRADE_BATCH_SIZE);const tickers=this.tickerQueue.splice(0,config.COLLECTOR_TICKER_BATCH_SIZE);
  ...
  }catch(e){this.recordFlushFailure(e);this.tradeQueue.splice(0,0,...[]);this.tickerQueue.splice(0,0,...[]);throw e;}
  finally{this.flushInProgress=false;}
 }
 updateStale(){
  const now=Date.now();const snap=this.ws?.snapshot?.();if(!snap)return;
  const tickers=this.ws.lastTickerByInstrument||new Map();
