...
function getPool() {
  ...
  if (!pool) pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: process.env.PGSSL === 'false' ? false : { rejectUnauthorized: false }, max: Number(process.env.PG_POOL_MAX || 5), idleTimeoutMillis: 30000, connectionTimeoutMillis: 8000 });
  return pool;
}
async function query(text, params=[]) { const p=getPool(); if(!p) throw new Error('DATABASE_URL is not configured'); return p.query(text, params); }
...
async function insertLiquidation(x){ if(!ready||x.timestamp==null) return; await query(`INSERT INTO liquidation_events(inst_id,ts,side,bk_px,size,raw) VALUES($1,to_timestamp($2/1000.0),$3,$4,$5,$6::jsonb)`,[x.symbol,x.timestamp,x.side,x.price,x.size,JSON.stringify(x)]); }
...
module.exports={enabled,getPool,query,migrate,ping,isReady,close,upsertInstruments,insertTicker,upsertCandle,insertTrade,insertOI,insertFunding,insertFundingHistory,insertOrderBook,insertLiquidation,insertAnomaly,cleanup};
