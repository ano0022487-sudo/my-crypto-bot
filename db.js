...
function getPool() {
  ...
  if (!pool) {
    pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: process.env.PGSSL === 'false' ? false : { rejectUnauthorized: false }, max: Number(process.env.PG_POOL_MAX || 5), idleTimeoutMillis: 30000, connectionTimeoutMillis: Number(process.env.PG_CONNECTION_TIMEOUT_MS || 10000) });
...
async function insertTickersBatch(rows){
  if(!ready||!rows.length)return;
  const values=[]; const params=[];
  rows.forEach((x,i)=>{const b=i*7;values.push(`($${b+1},to_timestamp($${b+2}/1000.0),$${b+3},$${b+4},$${b+5},$${b+6},$${b+7}::jsonb)`);params.push(x.symbol,x.timestamp,x.price,x.change24h,x.volume,x.volumeContracts,JSON.stringify(x));});
  await query(`INSERT INTO ticker_snapshots(inst_id,ts,price,change_24h,volume,volume_contracts,raw) VALUES ${values.join(',')}`,params);
...
async function insertTradesBatch(rows){
  if(!ready||!rows.length)return;
  const values=[]; const params=[];
  rows.forEach((x,i)=>{const b=i*8;values.push(`($${b+1},$${b+2},to_timestamp($${b+3}/1000.0),$${b+4},$${b+5},$${b+6},$${b+7},$${b+8}::jsonb)`);params.push(String(x.tradeId),x.instId,x.timestamp,x.price,x.size,x.side||null,x.source||null,JSON.stringify(x.raw||x));});
  await query(`INSERT INTO trades(trade_id,inst_id,ts,price,size,side,source,raw) VALUES ${values.join(',')} ON CONFLICT(inst_id,trade_id) DO NOTHING`,params);
  metrics.batchWrites+=1; metrics.batchRows+=rows.length;
}
...
module.exports={enabled,getPool,query,migrate,ping,isReady,stats,close,upsertInstruments,insertTicker,insertTickersBatch,upsertCandle,insertTrade,insertTradesBatch,insertOI,insertFunding,insertFundingHistory,insertOrderBook,insertLiquidation,insertAnomaly,cleanup};
