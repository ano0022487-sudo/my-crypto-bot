...
const safe = fn => async (req, res) => {
  ...
  catch (error) { logger.error('API route failed', { path: req.path, error: error.message }); res.status(503).json({ ok: false, error: '資料暫時無法取得' }); }
};
...
app.get('/api/markets', safe(async () => {
  ...
  const rows = tickers.map(t => ({ ...t, oi: oiMap.get(t.symbol)?.oiUsd ?? null, oiChange: oiMap.get(t.symbol)?.change ?? null, oiChangePct: oiMap.get(t.symbol)?.changePct ?? null }));
  return { count: rows.length, updatedAt: Date.now(), markets: rows };
}));
...
app.get('/api/funding-history/:instId', safe(async req => market.okx?.getFundingHistory ? market.okx.getFundingHistory(req.params.instId) : { status: '【資料不足，無法確認】' }));
